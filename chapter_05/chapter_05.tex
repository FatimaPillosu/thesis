...
The hyperparameter search space was defined as stated in Table .:

[Parameter 1]: Log-uniform distribution over [[min], [max]]
[Parameter 2]: Uniform distribution over [[min], [max]]
[Parameter 3]: Categorical distribution over [options]
[Parameter 4]: Integer uniform distribution over [[min], [max]]

Figure \ref{} illustrates the optimisation history for a representative outer fold, demonstrating the TPE algorithm's efficiency in identifying high-performing configurations. The solid lines track the trail with the "best value" for the objective. Convergence towards optimal values typically occurred within 10 and 15 trials. Optuna run the full set of 20 trials for all outer loops as shown in Figure \ref{}.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{dummy_optuna.png}
...
The final model configuration was selected based on the performance metrics across all cross-validation folds. Figure \ref{fig:hydro_based_ff_cross_validation_optuna_overall_scores} summarises the overall performance statistics for all the considered models, highlighting the superior stability and performance of the XGBoost implementation of the gradient boosting model. 

For each outer loop, the figure shows the average values obtained for each score in the inner loops. Overall, the decision-tree-based models provide a better performance than the neural networks. 

No trends in model performance across the chronologically ordered outer folds is observed, suggesting stationarity in the underlying relationships.  
