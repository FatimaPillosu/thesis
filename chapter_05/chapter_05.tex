...
\label{flash_flood_focused_verification_rainfall_based_ff_preprocessing}
 ...
 while the forecast data (i.e., ERA5-ecPoint rainfall forecasts) is .


\subsubsection{Defining \textit{verifying rainfall thresholds} for rainfall-based predictions of areas at risk of flash floods}

Given \marginpara{Definition of \textit{verifying rainfall threshold} in objective verification} a rainfall forecast value for a grid-box (e.g., in mm/24h), a flash flood event is considered a yes-event if the forecast exceeds a specific event threshold (defined in the same units of the forecasts, i.e., in mm/24h). In this case, the grid-box is assigned the value 1; otherwise, it is assigned the value 0. In objective verification, such an event threshold is called \textit{verifying rainfall threshold}.

Verifying \marginpara{Options to compute verifying rainfall thresholds using point-scale rainfall observations or blended/modelled gridded rainfall: pros and cons} rainfall thresholds of practical interest may be known (e.g., in my region, 50 mm/24h is likely to cause flash floods), or may be computed using historical observational or modelled rainfall estimates. Percentiles or return periods derived from such a rainfall distribution define the verifying thresholds. Higher percentiles or return periods correspond to rarer, more intense rainfall events, which are more likely to produce severe flash flooding. Computing verifying rainfall thresholds from long timeseries (e.g., at least 20 years) of high-density (e.g., rain gauges spaced a few kilometres apart) point rainfall observations (e.g., rain gauges or radar) yields the most accurate representation of the "true" value of localised extreme rainfall that is likely to cause flash floods \citep{Haiden_2016, RamosFilho_2021}. In the absence of a suitable observational network, the verifying rainfall thresholds may be defined from gridded rainfall products such as reanalysis, e.g., ERA5 \citep{Hersbach_2020}, reforecasts \citep{Hamill_2006b}, or blended rainfall observations provided on a grid such as MSWEP \citep{Beck_2019} or GPCP \citep{Adler_2018}. These datasets, however, tend to underestimate localised rainfall extremes due to their coarse spatial resolution \citep{Tapiador_2019}. 

In \marginpara{Verifying rainfall thresholds are computed using rainfall estimates from ERA5-ecPoint reanalysis to match the spatial resolution of the rainfall-based flash flood predictions of areas at risk of flash floods} this chapter, ERA5-ecPoint rainfall forecasts (already described in Section \ref{datasets_ecpoint}) are used to create the rainfall-based predictions of areas at risk of flash floods. In this case, historical modelled, gridded rainfall data should not be used to compute the verifying rainfall thresholds because their spatial resolution is incompatible with that of the ERA5-ecPoint forecasts given at point-scale. Gridded values represent averages over the model grid-box and are, therefore, typically lower than. If verifying rainfall thresholds were derived from such a gridded dataset, the point-scale forecasts would exceed them too often, giving a misleading impression that forecasts overpredict the risk of flash floods (i.e., there would be too many false alarms). To maintain consistency with the spatial resolution of the ERA5-ecPoint rainfall forecasts, the verifying rainfall thresholds should be computed using point-scale rainfall observations. In the absence of a long timeseries of high-density rain gauge observations over the CONUS, the verifying rainfall thresholds are computed with point-scale rainfall estimates from ERA5-ecPoint reanalysis \citep{Bottazzi_2024}, which have been shown to represent adequately climatologies computed with point-scale rainfall observations from rain gauges \citep{Pillosu_2025a}. The verifying rainfall thresholds were computed over the 1991–2020 period, following WMO best-practice for robust climatological analysis \citep{WMO_2017}.
...
\subsubsection{Creating accumulated, gridded observational fields matching the forecast data}
