...
\marginpara{Verification results: breakdown scores (ROC curves)}

The ROC curves demonstrate robust generalisation of discrimination ability across all models, with AUC values exhibiting minimal degradation between training and verification datasets. Most architectures maintain AUC-ROC values within 0.05 of their training performance, indicating stable ranking capabilities when distinguishing between flash flood and non-flood events. Random forest implementations show the most consistent performance across both datasets, whilst gradient boosting variants demonstrate slightly higher sensitivity to temporal shifts. Neural networks exhibit the greatest variability between training and verification performance, particularly evident in the curve shapes that suggest altered decision boundaries when confronted with the 2021-2024 data distribution.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{verif_training_test_breakdown_roc_curve.png}
\caption{\textbf{Verification results: breakdown scores (ROC curves)} }
\label{fig:verif_training_test_breakdown_reliability_diagram}
\end{figure}

The precision-recall curves reveal substantially greater performance degradation in minority class detection capabilities during verification. The sharp decline in precision at low recall values indicates that models struggle to maintain their positive predictive value when applied to temporally shifted data. This pattern is particularly pronounced for models trained with balanced loss functions, where the curves rapidly approach the baseline. Weighted loss functions demonstrate improved resilience, maintaining higher precision across broader recall ranges, though all models show compressed precision-recall trade-offs compared to training performance, reflecting the fundamental challenge of rare event detection in operational deployment.

\marginpara{Verification results: breakdown scores (Precision-Recall curves)}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{verif_training_test_breakdown_pr_curve.png}
\caption{\textbf{Verification results: breakdown scores (Precision-Recall curves)} }
\label{fig:verif_training_test_breakdown_reliability_diagram}
\end{figure}

The reliability diagrams expose systematic calibration shifts between training and verification periods. Models trained with balanced loss functions exhibit severe underconfidence, with predicted probabilities consistently lower than observed frequencies, manifesting as curves lying above the diagonal. Conversely, weighted loss functions produce overconfident predictions in the verification period, with curves falling below the diagonal. Gradient boosting implementations demonstrate the most extreme calibration degradation, particularly CatBoost, which shows erratic reliability patterns suggesting unstable probability estimates. These calibration issues indicate that whilst models maintain reasonable discrimination ability, their probability outputs require recalibration for operational use, particularly when temporal distribution shifts are present.

\marginpara{Verification results: breakdown scores (Reliability diagrams)}


\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{verif_training_test_breakdown_reliability_diagram.png}
\caption{\textbf{Verification results: breakdown scores (Reliability diagrams)} }
\label{fig:verif_training_test_breakdown_reliability_diagram}
\end{figure}
