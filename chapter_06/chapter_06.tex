...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{chapter_06/figures/cv_optuna.png}
\caption{\textbf{Repeated nested cross-validation for the training of data-driven models using (imbalanced) flash flood impact reports.} The workflow illustrates the hierarchical validation structure applied to the training dataset (purple block), which corresponds to data (forecasts and observations) from 2001 to 2020, containing 19.75 million gridded data points, of which only 53245 (0.27\%) correspond to yes-events. The outer cross-validation loop utilises the "RepeatedStratifiedKFold" function from the Python library Scikit-Learn. This function creates k_outer = 5 outer folds (grey blocks), maintaining the class distribution of the training dataset, over n_repeats = 1 time. Each outer fold was split into an outer training dataset (80\%, blocks in shades of pink and orange) and an outer test dataset (20\%, green blocks). Within each outer fold, a Bayesian hyperparameter tuning was performed employing the Python library Optuna. The hyperparameter tuning was performed through an inner cross-validation procedure. Over n_trial = 20 repetitions, the outer training dataset was split into k_inner = 3 inner folds. Using the inner training folds (pink blocks), each Optuna trial trains the model considering a candidate set of hyperparameters and evaluates it over the inner validation folds (orange blocks). The performance for the trial corresponds to the mean of the considered evaluation metric (AUC-ROC, area under the ROC curve and AUC-PR, area under the Precision-Recall curve) for the k_inner = 3 inner validation folds. The optimal hyperparameter set, identified by maximising the selected evaluation metric, is subsequently used to train the final model on the complete outer training subset. Model performance is assessed on the held-out outer test fold using the considered evaluation metrics (AUC-ROC and AUC-PR). The best-performing fold is retrained on the entire training dataset (purple block) for operational deployment. Independent, more extensive verification is performed using the verification dataset (cyan block), which corresponds to data from 2021 to 2024, containing 4.36 million gridded data points, of which only 10742 (0.25\%) correspond to yes-events. The extensive verification involves a more comprehensive evaluation of the data-driven predictions, considering the Precision-Recall curve and AUC-PR, the ROC curve and AUC-ROC, reliability diagrams, and frequency bias.}
\label{fig:cv_optuna}
\end{figure}
